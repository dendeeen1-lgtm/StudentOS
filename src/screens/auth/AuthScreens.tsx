      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <FadeInView delay={150} style={styles.form}>
            ...
            <Input label="LRN (Learner Reference Number)" value={form.lrn} onChangeText={set('lrn')} placeholder="123456789012" keyboardType="numeric" />
            <Input label="School" value={form.school} onChangeText={set('school')} placeholder="Name of your school" />
            <Input label="Home address" value={form.address} onChangeText={set('address')} placeholder="Street, City, Province" multiline numberOfLines={2} />
...
export const RegisterParentScreen: React.FC<Props> = ({ navigation }) => {
  const [form, setForm] = useState({ fullName: '', relationship: 'Parent', mobileNumber: '', email: '', password: '' });
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const set = (key: string) => (val: string) => setForm((f) => ({ ...f, [key]: val }));
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      await registerParent(email, password, { fullName, mobileNumber, relationship });
  ...
  <SafeAreaView style={styles.container}>
    <FadeInView style={styles.pendingWrap}>
      ...
      <Text style={styles.pendingSub}>
        Your account has been submitted. Your adviser or class monitor needs to approve it before you can start using StudentOS.
      </Text>
      <Text style={styles.pendingHint}>You'll be able to sign in once approved.</Text>
